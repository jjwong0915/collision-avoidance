...
kitti_depth_folder = (
    "/home/mjchiu/Documents/darknet-depth/dataset/KITTI/depth_interpol/"
)
...
city_image_folder = (
    "/home/mjchiu/Documents/darknet-depth/dataset/AirSim/CityEnviron/Train/"
)
city_depth_folder = (
    "/home/mjchiu/Documents/darknet-depth/dataset/AirSim/CityEnviron/Train/"
)
escenter_image_folder = (
    "/home/mjchiu/Documents/darknet-depth/dataset/NCTU/ES_building/Train/"
)
escenter_depth_folder = (
    "/home/mjchiu/Documents/darknet-depth/dataset/NCTU/ES_building/Train/"
)
NYUv1_mat_data_path = (
    "/home/mjchiu/Documents/darknet-depth/dataset/NYU_depth/nyu_depth_data_labeled.mat"
)
NYUv2_mat_data_path = (
    "/home/mjchiu/Documents/darknet-depth/dataset/NYU_depth/nyu_depth_v2_labeled.mat"
)
NYUv2_raw_data_path = (
    "/home/mjchiu/Documents/darknet-depth/dataset/NYU_depth/NYUv2_raw_data/"
)
...
def berhu_loss(predict, target):
    ...
    depth_loss = tf.reduce_sum(depth_loss) / (
        tf.count_nonzero(target, dtype=tf.float32) + 0.000001
    )
    return depth_loss
...
def getCityscapesPath(path):
    ...
    for dirname, dirnames, filenames in os.walk(path):
        for filename in filenames:
            ...
            if (
                "labelIds" in imgPath
                and "train" in imgPath
                and "troisdorf_000000_000073" not in imgPath
            ):
                dataList.append(imgPath)
...
def depth_read_KITTI(filename, shape=(DEPTH_H, DEPTH_W), rand=-1):
    ...
    depth_png = np.array(
        Image.open(filename).resize((DEPTH_W, DEPTH_H), Image.NEAREST), dtype=int
    )
    # make sure we have a proper 16bit depth map here.. not 8bit!
    assert np.max(depth_png) > 255

    depth = depth_png.astype(np.float) / 255.0
...
def read_KITTI_PSMNet_depth(
    image_path,
    error_mean_var,
    shape=(DEPTH_H, DEPTH_W),
    rand=-1,
    include_sky=False,
    ...
    with open(image_path, "rb") as file:
        ...
        if rand == -1:
            ...
            depth = cv2.resize(
                depth, (int(DEPTH_H * depth.shape[1] / depth.shape[0]), DEPTH_H)
            )
    ...
    if include_sky:
        ...
        seg_map = cv2.resize(
            seg_map, (DEPTH_W, DEPTH_H), interpolation=cv2.INTER_NEAREST
        )
    ...
    if log_depth:
        ...
        depth = (
            (np.log2(depth) - np.log2(depth_min))
            * depth_max
...
def load_seg_data_cityscapes(seg_path, shape, CLASSES=19):
    ...
    for i in range(19):
        seg_one_hot[:, :, i] = np.where(
            seg_map[:, :, 0] == ID[i], ones, seg_one_hot[:, :, i]
        )
...
def read_Cityscapes_depth_from_disparity(image_path, shape=(DEPTH_H, DEPTH_W)):
    ...
    disparity = cv2.resize(
        disparity, (shape[1], shape[0]), interpolation=cv2.INTER_NEAREST
    )
...
def load_seg_data(seg_path, shape, CLASSES=19, rand=-1):
    ...
    if rand == -1:
        seg_map = cv2.resize(
            seg_map, (shape[1], shape[0]), interpolation=cv2.INTER_NEAREST
        )
    else:
        seg_map = cv2.resize(
            seg_map, (int(IMAGE_H * seg_map.shape[1] / seg_map.shape[0]), IMAGE_H)
        )
...
def UltraHybridGenerator_multiloss(
    dataList,
    shape=[
        (DEPTH_H, DEPTH_W),
        (DEPTH_H, DEPTH_W),
    ...
    while 1:
        for i in range(int(len(dataList) / BATCH_SIZE)):  # of batches
            ...
            depth = [
                np.zeros((BATCH_SIZE, shape[0][0], shape[0][1], 1)),
            ...
            seg = [
                np.zeros((BATCH_SIZE, shape[0][0], shape[0][1], CLASSES)),
            ...
            for j in range(i * BATCH_SIZE, (i + 1) * BATCH_SIZE):
                ...
                if "AirSim" in dataList[j]:
                    if "CityEnviron" in dataList[j]:
                        ...
                        img[j - i * BATCH_SIZE] = read_AirSim_image(
                            imgPath, shape=(int(shape[0][0] * 2), int(shape[0][1] * 2))
                        )
                    else:
                        subPath = depthPath.split(os.sep)
                        imgPath = (
                    ...
                    img[j - i * BATCH_SIZE] = read_KITTI_image(
                        imgPath,
                        shape=(int(shape[0][0] * 2), int(shape[0][1] * 2)),
                        rand=rand_init,
                    )
                    ...
                    if "gtFine" in depthPath:
                        ...
                        image_path = depthPath.replace(
                            "gtCoarse_labelIds", "leftImg8bit"
                        )
                        image_path = image_path.replace("gtCoarse", "leftImg8bit")
                    img[j - i * BATCH_SIZE] = read_KITTI_image(
                        image_path, shape=(int(shape[0][0] * 2), int(shape[0][1] * 2))
                    )
                ...
                for size_iter in range(len(depth)):
                    if "AirSim" in dataList[j]:
                        if "CityEnviron" in dataList[j]:
                            depth[size_iter][j - i * BATCH_SIZE] = read_AirSim_depth(
                                depthPath, shape=shape[size_iter]
                            )
                        else:
                            depth[size_iter][j - i * BATCH_SIZE] = read_AirSim_depth(
                                depthPath, shape=shape[size_iter]
                        ...
                        if not train_airsim_with_deeplabv3:
                            ...
                            seg[size_iter][j - i * BATCH_SIZE] = load_seg_data_airsim(
                                seg_path, shape[size_iter], CLASSES=CLASSES
                            )
                        elif train_airsim_with_deeplabv3:
                            seg_path = depthPath.replace("pfm", "png")
                            seg_path = seg_path.replace("depthPlanner", "seg_deeplabV3")
                            seg[size_iter][j - i * BATCH_SIZE] = load_seg_data(
                                seg_path, shape[size_iter]
                            )

                    elif "KITTI" in dataList[j]:
                        if "depth_SPNet" in dataList[j]:
                            if mixGroundTruth:
                                depth_PSMNet = read_KITTI_PSMNet_depth(
                                    depthPath,
                                    error_mean_var,
                                    shape=shape[size_iter],
                                    rand=rand_init,
                                    include_sky=include_sky,
                                    depth_crrection=depth_crrection,
                                    log_depth=True,
                                )

                                depthPath_interpol = depthPath
                                depthPath_interpol = depthPath_interpol.replace(
                                    "depth_SPNet", "depth"
                                )
                                depthPath_interpol = depthPath_interpol.replace(
                                    "pickle", "png"
                                )
                                depth_gt = depth_read_KITTI(
                                    depthPath_interpol,
                                    shape=shape[size_iter],
                                    rand=rand_init,
                                )
                                ...
                                depth_gt = np.where(
                                    depth_gt < 0, depth_PSMNet, depth_gt
                                )
                                depth[size_iter][j - i * BATCH_SIZE] = depth_gt
                            else:
                                depth[size_iter][
                                    j - i * BATCH_SIZE
                                ] = read_KITTI_PSMNet_depth(
                                    depthPath,
                                    error_mean_var,
                                    shape=shape[size_iter],
                                    rand=rand_init,
                            ...
                            depth[size_iter][
                                j - i * BATCH_SIZE
                            ] = read_KITTI_interpol_depth(
                                depthPath, shape=shape[size_iter], rand=rand_init
                            )
                    elif "CityScape" in dataList[j]:
                        depth[size_iter][j - i * BATCH_SIZE] = read_Cityscapes_depth(
                            depthPath, shape=shape[size_iter]
                        )
                        seg[size_iter][j - i * BATCH_SIZE] = load_seg_data_cityscapes(
                            depthPath, shape[size_iter], CLASSES=CLASSES
                        )
...
def plot_class_sample(seg_pred, segment_type=19):
    ...
    for i in curr_class:
        ...
        cv2.rectangle(
            img,
            (400 * (counter % 5), 100 * int(counter / 5)),
            (400 * (counter % 5) + 100, 100 * int(counter / 5) + 100),
            labels[i].color,
            -1,
        )
        cv2.putText(
            img,
            labels[i].name,
            (400 * (counter % 5) + 120, int(100 * (int(counter / 5) + 0.6))),
...
def decode_precition(seg_pred, segment_type=19):
    ...
    for i in range(19):
        seg_vis = np.where(
            seg_argmax == [i, i, i], np.array(labels[i].color) * ones, seg_vis
        )
    # print(seg_vis.shape)
    return seg_vis
...
def compute_threshold(y_predict, y_true, thida=1.25):
    ...
    threshold = (len(maxValue[maxValue < thida]) - len(maxValue[maxValue == 0])) / len(
        maxValue[maxValue > 0]
    )
...
def model_evaluation(model, dataList, totalData, image_shape, log_depth=False):
    ...
    for dataIter, data in enumerate(dataList[:totalData]):
        ...
        if log_depth:
            ...
            results = 2 ** (
                results * (np.log2(depth_max) - np.log2(depth_min)) / depth_max
                + np.log2(depth_min)
            )
...
def model_evaluation_single_output(
    model, dataList, totalData, image_shape, log_depth=False
):
    rmse_all, rmse_50, rmse_30, ARD, SRD, threshold_1, threshold_2, threshold_3 = (
        [],
        [],
