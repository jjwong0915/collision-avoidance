...
def _depthwise_conv_block_v2(inputs, pointwise_conv_filters, alpha, expansion_factor,depth_multiplier=1, strides=(1, 1), bn_epsilon=1e-3,bn_momentum=0.999, block_id=1):
...
# - 說明：用來存放模型的layer design
# - Input：
#      (tuple) inputShape: model的input resolution
#      (float) alpha: MobileNetV2可調控的參數設定，可設為1.0, 0.75, 0.5, 0.35，詳見MobileNet的paper
#      (int) expansion_factor: MobileNetV2可調控的參數設定，詳見MobileNetV2的paper
#      (int) depth_multiplier: MobileNetV2可調控的參數設定，詳見MobileNetV2的paper
#      (bool) lock_backend_weights: 設為True則只會針對model中的decoder進行訓練。設為False則會針對整個model(包含encoder和decoder)做訓練
#      (int) CLASSES: model產生semantic segmentation的class數量
# - Output：一個型態為Keras.models的Model，可在training和inference時載入
def MNv2_segment_depth_multiloss_model(inputShape=(416,416,3), alpha=1.0, expansion_factor=6, depth_multiplier=1, lock_backend_weights = True, CLASSES = 19):
    ...
    model_backend = Model(inputs=mobilenetv2.input,outputs=[mobilenetv2.get_layer('block_16_project_BN').output, #(13,13,320)
                                                            ...
                                                            mobilenetv2.get_layer('expanded_conv_project_BN').output]) #(208,208,16)
    ...
    x = _depthwise_conv_block_v2(x, 64, alpha, expansion_factor, depth_multiplier, block_id=1)
    x = _depthwise_conv_block_v2(x, 64, alpha, expansion_factor, depth_multiplier, block_id=2)
    x = _depthwise_conv_block_v2(x, 64, alpha, expansion_factor, depth_multiplier, block_id=3)
    x = _depthwise_conv_block_v2(x, 64, alpha, expansion_factor, depth_multiplier, block_id=4)
    ...
    x = _depthwise_conv_block_v2(x, 32, alpha, expansion_factor, depth_multiplier, block_id=7)
    x = _depthwise_conv_block_v2(x, 32, alpha, expansion_factor, depth_multiplier, block_id=8)
    x = _depthwise_conv_block_v2(x, 32, alpha, expansion_factor, depth_multiplier, block_id=9)
    ...
    x = _depthwise_conv_block_v2(x, 24, alpha, expansion_factor, depth_multiplier, block_id=12)
    x = _depthwise_conv_block_v2(x, 24, alpha, expansion_factor, depth_multiplier, block_id=13)
    x = _depthwise_conv_block_v2(x, 24, alpha, expansion_factor, depth_multiplier, block_id=14)
    ...
    x = _depthwise_conv_block_v2(x, 16, alpha, expansion_factor, depth_multiplier, block_id=17)
    x = _depthwise_conv_block_v2(x, 16, alpha, expansion_factor, depth_multiplier, block_id=18)
    ...
    for i in range(len(model_backend.layers),len(model.layers)):
        model.layers[i].kernel_initializer = initializers.RandomNormal(mean=0.0, stddev=0.05, seed=None)
